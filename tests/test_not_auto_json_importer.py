...
def test_load_json_txt_fullmock_grammar_b() -> None:
    path = REPO_ROOT / "NOT_auto_json" / "Full_Mock" / "05_Grammar_B.txt"
...
def test_load_json_txt_reading_short() -> None:
    path = REPO_ROOT / "NOT_auto_json" / "Excecise" / "Reading_Short" / "01_Reading_short.txt"
    obj = load_json_txt(path)
...
def test_build_full_parts_payload_shape() -> None:
    set_dir = REPO_ROOT / "NOT_auto_json" / "Full_Mock"
...
def test_build_practice_payload_listening_b_fix_applied() -> None:
    path = REPO_ROOT / "NOT_auto_json" / "Excecise" / "Listening_B" / "01_Listening_B.txt"
