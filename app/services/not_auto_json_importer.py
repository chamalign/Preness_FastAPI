"""
NOT_auto_json/*.txt (GPT 中間形式) を FastAPI import API 用 payload に変換する。
...
def load_json_txt(path: Path) -> Dict[str, Any]:
    """
    NOT_auto_json の .txt を JSON として読み込む。
...
def normalize_fullmock_title(dir_name: str, *, kind: str) -> str:
    ...
    現状は `NOT_auto_json/Full_Mock/` と `NOT_auto_json/Short_Mock/` が 1 セットだが、
