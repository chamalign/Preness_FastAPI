...
上図は主にキー・関連のみを示す。display_order, passage, audio_url, タイムスタンプ等の各カラムはテーブル詳細を参照すること。
...
タイトルは持たない。表示名は、関連する section の section_type、part の part_type および display_order から自動生成する。
...
| audio_url | VARCHAR | NULL | 音声ファイルのS3 URL（APIの conversation_audio_url に相当） |
| display_order | INTEGER | NOT NULL | |
| created_at | TIMESTAMP | NOT NULL | |
| updated_at | TIMESTAMP | NOT NULL | |
...
APIの `tag` / `wrong_reason_*` に対応するカラムを含む。
...
| audio_url | VARCHAR | NULL | 設問用音声のS3 URL（APIの question_audio_url に相当） |
| choice_a | TEXT | NOT NULL | |
| choice_b | TEXT | NOT NULL | |
| choice_c | TEXT | NOT NULL | |
| choice_d | TEXT | NOT NULL | |
| correct_choice | ENUM | NOT NULL | A, B, C, D。APIは小文字 (a/b/c/d) で送る。保存時に A/B/C/D に正規化して格納する。 |
| explanation | TEXT | NULL | 解説 |
| tag | VARCHAR | NULL | 問題タグ（APIの tag に相当。例: 推測, 詳細, 主旨） |
| wrong_reason_a | TEXT | NULL | 選択肢Aの誤答理由（APIの wrong_reason_a に相当） |
| wrong_reason_b | TEXT | NULL | 選択肢Bの誤答理由（APIの wrong_reason_b に相当） |
| wrong_reason_c | TEXT | NULL | 選択肢Cの誤答理由（APIの wrong_reason_c に相当） |
| wrong_reason_d | TEXT | NULL | 選択肢Dの誤答理由（APIの wrong_reason_d に相当） |
| created_at | TIMESTAMP | NOT NULL | |
| updated_at | TIMESTAMP | NOT NULL | |
...
> **やり直し:** 同一ユーザー・同一モック/演習の再受験時は、同一 attempt を再利用する（completed_at を NULL にし、answers を上書きする等）。新規 attempt は作成しない。
...
| selected_choice | ENUM | NULL | A, B, C, D（未回答はNULL）。入力が小文字の場合は保存前に A/B/C/D に正規化する。 |
