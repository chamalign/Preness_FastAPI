...
├── API _regulations/
│   ├── API仕様書.md
│   ├── Horiba_based_FastAPI.md
│   └── ...                     # FastAPI連携_API設計書.md, データベース設計.md など
├── docs/
│   ├── analysis_api.md
│   └── analysis_short_api.md
├── prompts/
...
| **POST** /api/v1/analysis/short/jobs | Rails（Short 模試） | 85 問 `items[]`, `passages` 2×10, `goal_score` null 可 等 | 202 + `{ job_id, job_type: "short", status }`. 仕様は [docs/analysis_short_api.md](docs/analysis_short_api.md) |
...
#### NOT_auto_json txt 投入（CLI）
`NOT_auto_json/**/*.txt` から `full_parts` / `part_data` を組み立て、`/api/v1/import/*` へ直投入する。

FM（full_mock）:
```bash
python scripts/not_auto_json_import.py --api-key "$CONTENT_SOURCE_API_KEY" import-full-mock
```
`NOT_auto_json/Full_Mock_02` のようにインクリメントしたセットも投入したい場合は `set_dir` を指定する:
```bash
python scripts/not_auto_json_import.py --api-key "$CONTENT_SOURCE_API_KEY" import-full-mock "NOT_auto_json/Full_Mock_02"
```

SM（short_mock）:
```bash
python scripts/not_auto_json_import.py --api-key "$CONTENT_SOURCE_API_KEY" import-short-mock
```
`Short_Mock_02` なども同様に `set_dir` を指定:
```bash
python scripts/not_auto_json_import.py --api-key "$CONTENT_SOURCE_API_KEY" import-short-mock "NOT_auto_json/Short_Mock_02"
```

P（practice）: ファイル指定
```bash
python scripts/not_auto_json_import.py --api-key "$CONTENT_SOURCE_API_KEY" import-practice \
  --file "NOT_auto_json/Excecise/Listening_A/01_Listening_A.txt"
```

P（practice）: Reading の未使用のみランダム投入（Short/Long の候補から選ぶ）
```bash
python scripts/not_auto_json_import.py --api-key "$CONTENT_SOURCE_API_KEY" import-practice --reading-random
```
このとき未使用判定はデフォルトで `outputs/used_reading.json` を使う。必要に応じて以下を指定できる:
...
- [API _regulations/API仕様書.md](API%20_regulations/API仕様書.md) … 問題投入 API のリクエスト/レスポンススキーマ
- [docs/analysis_api.md](docs/analysis_api.md) … Full 分析 API
- [docs/analysis_short_api.md](docs/analysis_short_api.md) … Short 模試分析 API（85 問・latest・passages）
